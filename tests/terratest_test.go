...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-sol-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Serial over LAN Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/sol/Policies/%s", moid), expectedJSONTemplate, vars)
}
